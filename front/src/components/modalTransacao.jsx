...
import {
  Utensils, Car, Home, Gamepad2, HeartPulse, BookOpen,
  ShoppingCart, BadgeDollarSign, Wallet, TrendingUp,
  Banknote, Gift, FileText, Plane, PawPrint, MoreHorizontal,
  ChevronDown, Check
} from "lucide-react";

export function ModalTransacao({ isOpen, onClose }) {
  ...
  const handleCategoriaChange = (value) => {
    const selecionada = transacoesTipo.find((item) => item.value === value);
  ...
  const cadastrarTransacao = async (e) => {
    e.preventDefault();
    try {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-500/30 z-50">
      <div className="bg-white rounded-lg !p-6 lg:!p-8 w-[400px] sm:w-[510px] lg:w-[640px] xl:w-[800px] shadow-lg">
        <h1 className="text-[20px] text-[#1A2B4C] font-medium">Nova Transação</h1>
        <form onSubmit={cadastrarTransacao}>

          <div className="flex flex-col items-start w-full !mt-3">
            <h1 className="font-bold text-[15px] text-[#1A2B4C]">Tipo</h1>

            <div className="flex gap-5">
              <div className="flex items-center justify-center gap-2">
                <input type="radio" id="entrada" name="tipo" value="entrada" checked={formData.tipo === "entrada"} onChange={handleChange} />
                <label htmlFor="entrada" className="font-bold text-[15px] text-[#1A2B4C]">Entrada</label>
              </div>

              <div className="flex items-center justify-center gap-2">
                <input type="radio" id="saida" name="tipo" value="saida" checked={formData.tipo === "saida"} onChange={handleChange} />
                <label htmlFor="saida" className="font-bold text-[15px] text-[#1A2B4C]">Saída</label>
              </div>
            </div>
          </div>

          <div className="flex flex-col items-start w-full !mt-3">
            <label htmlFor="valor" className="font-bold text-[15px] text-[#1A2B4C]">Valor:</label>
            <div className="relative w-full">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">R$</span>
              <input type="number" name="valor" id="valor" placeholder="0,00" value={formData.valor} onChange={handleChange} required
          ...
          <div className="flex flex-col items-start w-full !mt-3">
            <label htmlFor="data" className="font-bold text-[15px] text-[#1A2B4C]">Data:</label>
            <input type="date" name="data" id="data" value={formData.data} onChange={handleChange}
              className="!p-2 border border-gray-300 w-full rounded shadow text-gray-500" required />
          </div>

          <div className="flex flex-col items-start w-full !mt-3">
            <label htmlFor="descricao" className="font-bold text-[15px] text-[#1A2B4C]">Descrição:</label>
            <input type="text" name="descricao" id="descricao" placeholder="nome..." value={formData.descricao} onChange={handleChange}
              className="!p-2 border border-gray-300 w-full rounded shadow text-gray-500" required />
          </div>
